...
sap.ui.define([
	...
	return BaseController.extend("be.deloitte.dce.dceclassified.controller.Object", {
		...
		_onObjectMatched: function (oEvent) {
			...
			var sProduct = oEvent.getParameter("arguments").productGUID;
			var sScheme = oEvent.getParameter("arguments").scheme;
			this.getModel().metadataLoaded().then(function () {
				var sObjectPath = this.getModel().createKey("Classifications", {
					GUID: sObjectId
				});
			...
			this.getView().setModel(oFilterAttrsModel, "filterCriteriaAttrsModel");
			var oOtherAttrsModel = new JSONModel();
			oOtherAttrsModel.setData(aOtherAttrs);
			this.getView().setModel(oOtherAttrsModel, "otherAttrsModel");
				...
				success: function(oData) {
					oModel.read("/ClientAttributes", {
						success: function(oData2) {
							...
							that.getView().getModel("filterCriteriaAttrsModel").setData(aFilterCriteriaAttrs);
							that.getView().getModel("otherAttrsModel").setData(aOtherAttrs);
						}
					});
				}
			...
			var aFilters3 = [];
			var aSchemeFilters = [];
			aFilters3.push(new Filter({
				path: "ProductGUID",
				operator: sap.ui.model.FilterOperator.EQ,
				value1: sProduct
			}));
			aSchemeFilters.push(new Filter({
				path: "Scheme",
				operator: sap.ui.model.FilterOperator.EQ,
				value1: sScheme
			}));
			...
			aFilters3.push(new Filter(aSchemeFilters, false));
			this.byId("idCommentsList").getBinding("items").filter(aFilters3);
		...
		onPost: function (oEvent) {
			...
			var that = this;
			var bCommentAllSchemes = this.byId("idCommentAllSchemesCheckBox").getSelected();
			...
			if(!bCommentAllSchemes) {
				bodyJSON.Scheme = Product.Scheme;
			}
			...
			oModel.attachRequestFailed(function (oResponse) {
				MessageBox.error("Error when saving the comment!");
			});
		},
